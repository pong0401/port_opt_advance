...
SNP_GUIDE_CONFIGS = {
    ...
    "Country ETF tactical + Gold DD boost 16": {
        ...
        "description": "Strategy A แบบ tactical ที่เริ่มจาก SPY/Gold/BTC/BIL แล้วแบ่งทุนจาก SPY ไปยัง momentum ETF และ country ETF พร้อม Gold boost และ daily exposure แบบ lag-1.",
        ...
        "guide_bullets": [
            "Base allocation ก่อนจัด tactical sleeves: SPY 45%, Gold 30%, BTC 10% และ BIL 15%.",
            "Base momentum sleeve: เลือก ETF อันดับ 1 จาก SPMO, MTUM, SCHG, XLK, EWY, EWJ และ INDA โดยต้องมี momentum 3 เดือนและ 6 เดือนเป็นบวก พร้อมราคาอยู่เหนือ MA200; จัดสรร 15% โดยหักทุนจาก SPY.",
            "Country ETF sleeve: เลือก country ETF อันดับสูงสุด 2 ตัวด้วยคะแนน momentum รายเดือนจาก country-only universe; จัดสรรรวม 8% หรือเท่ากันตัวละ 4% โดยหักทุนจาก SPY.",
            "Universe ของ country ETF ต้องมี usable price history ประมาณ 10 ปี ส่วนรายชื่อที่เลือกจะคำนวณใหม่จากข้อมูลล่าสุดทุกครั้งที่ refresh.",
            "Optimizer/model: เป็น rules-based monthly momentum allocation ไม่ใช้ mean-variance optimizer; parameter ชุดนี้เป็นผู้ชนะจากการ sweep โดยเลือก Sharpe สูงสุดใน BEST_PARAM S&P family.",
            "Rebalance schedule: เลือก momentum และ country ETFs ที่ month-end แล้วถือ target weights จนถึง rebalance ถัดไป; signal ที่อาศัยราคาปิดใช้ lag-1 สำหรับ next-session execution.",
            "Caps: country ETF satellite รวม 8%, momentum ETF 15%, Gold base 30% และเพิ่มได้อีกสูงสุด 16% จาก SPY; หลัง funding/boost ต้องเหลือ SPY อย่างน้อย 20%, ขณะที่ BTC และ BIL มี base weight 10% และ 15%.",
            "Latest weights คำนวณใหม่ด้วย scripts/refresh_country_etf_tactical_latest.py จาก cache และ market data ใน repo นี้; historical curve ใช้ BEST_PARAM S&P Step 4 column current_best country_only bucket8% top2 boost16.",
        ],
        "daily_exposure_bullets": [
            "ใช้ daily exposure overlay กับ SPY, Gold และ BTC เท่านั้น; country ETFs, momentum ETF และ BIL ยังคง active ตาม target weights จนถึง rebalance ถัดไป.",
            "Signal timing เป็น close-based lag-1: สัญญาณจากราคาปิดที่ทราบแล้วถูกใช้กับ session ถัดไป เพื่อไม่ให้เกิด lookahead.",
            "SPY: ถ้าราคาต่ำกว่า MA300 จะลด exposure เหลือ 50%; ส่วนที่ลดลงไปอยู่ใน Cash / Reduced Exposure.",
            "Gold: ใช้ DD252; เมื่อ drawdown <= -8% จะลด exposure เหลือ 50% และเมื่อ drawdown <= -20% ยังคง exposure ที่ 50%; ส่วนที่ลดลงไปอยู่ใน Cash / Reduced Exposure.",
            "BTC: ถ้าราคาต่ำกว่า MA50 จะลด exposure เหลือ 0%; ส่วนที่ลดลงไปอยู่ใน Cash / Reduced Exposure.",
            "Gold boost เป็นกฎระดับ rebalance แยกจาก daily exposure: ถ้า SPY ต่ำกว่า MA200 หรือ SPY DD252 <= -8% จะเพิ่ม Gold สูงสุด 16% โดยหักจาก SPY ก่อนนำ daily exposure มาใช้.",
        ],
    },
...
STRATEGY_B_GUIDE_CONFIGS = {
    "Tactical TH/Gold/BTC 60/30/10 asset-level daily exposure": {
        "series": "Tactical TH/Gold/BTC 60/30/10 asset-level daily exposure",
        "latest_weights_file": "result/us_th_tactical_perf_momentum_603010_latest_effective_security_weights_thb.csv",
        "latest_weights_metadata_file": "result/us_th_tactical_perf_momentum_603010_latest_meta.json",
        "latest_weights_strategy": "Tactical TH/Gold/BTC 60/30/10 asset-level daily exposure",
        "latest_refresh_scripts": ["scripts/refresh_us_th_tactical_603010_latest.py"],
        "description": "????? Tactical TH ????????? Equity/Gold/BTC 60/30/10 ?????? asset-level daily exposure ??? lag-1 ??? exposure ??????????????? Cash / Reduced Exposure.",
        "setting": "Equity 60% / Gold 30% / BTC 10% / US PIT optimized sleeve THB + TH PIT optimized sleeve THB / tactical TH proxy_regime relative_return binary lb1 cap30 / asset-level daily exposure lag-1 / reduced exposure to cash",
        "guide_bullets": [
            "Strategy setup",
            "Base allocation: Equity 60%, Gold 30%, BTC 10%.",
            "Universe: equity sleeve ??? US PIT optimized sleeve THB ??? TH PIT optimized sleeve THB; Gold ??? GC=F ??? BTC ??? BTC-USD ??????????????????????? THB.",
            "Selection rules: ????????? US ??? TH sleeves ????? PIT universe ??? Dynamic HMM Copula mean-variance model ??????? momentum; ????? 30 ????????????? sleeve ??? cap ?????? 8% ????? sleeve.",
            "Tactical TH model: ??? proxy_regime ??????????????????? 1 ???????? S&P 500 ETF THB ??? SET Index THB proxy ??? relative_return ??? binary allocation.",
            "TH gate: entry 0%, exit 0%, min hold 0 ?????, confirm 1 ????? ??? lag ???????????????????????; ????? gate on ???????????????? 30% ??? equity sleeve ???? 18% ??????????? ???? equity ?????????????? US sleeve.",
            "Objective: ??? US ???? equity core ??????? TH sleeve ????? SET proxy ??????????? S&P 500 ETF ??????? THB ????? Gold/BTC ???? diversifiers.",
            "Rebalance schedule: ??????????? PIT optimized sleeve rebalance ????????; TH tactical gate ???????????????????????? ??? daily exposure ???? effective weights ????????????? rebalance.",
            "Caps: Equity ??? 60%, TH ?????? 18% ???????????, Gold 30%, BTC 10% ??????????????? cap 8% ????? sleeve ??????? sleeve weight/exposure.",
            "Latest weights ????? scripts/refresh_us_th_tactical_603010_latest.py ???????????????? data/cache ??? fresh market panel ??? repo ??? ?????????? static latest-weight files ??? dynamic_port_opt.",
        ],
        "daily_exposure_bullets": [
            "Daily exposure rules",
            "??? asset-level daily exposure ????????? raw sleeve weights ????; signal ??? close ??? session ??????????? shift 1 ?????????????? next-session execution.",
            "US Equity: ??? SPY MA300; ???????????????? MA300 exposure ????? 50%.",
            "TH Equity: ??? SET MA200; ???????????????? MA200 exposure ????? 0%.",
            "Gold: ????? Gold MA50 ??? below-MA exposure ?? config ????????????? 100% ??????? Gold ???????? exposure.",
            "BTC: ??? BTC MA50; ???????????????? MA50 exposure ????? 0%.",
            "Exposure ??????????? US, TH ???? BTC ????????? risky sleeve ???? ??????????? Cash / Reduced Exposure.",
        ],
        "metrics": {"CAGR": 0.17671815123921242, "Sharpe": 0.9825779941400987, "Max Drawdown": -0.18417007021845822, "Total Return": 24.55993126363642},
    },
