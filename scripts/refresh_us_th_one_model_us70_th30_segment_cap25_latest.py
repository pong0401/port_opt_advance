...
refresh.STRATEGY = (
    ...
    "combined segment cap 25% + daily exposure"
)
refresh.CASE = "US cap 70% / TH cap 30% stockcap5 penalty0.02 assets50 combined segment cap 25%"
refresh.RESULT_PREFIX = "us_th_one_model_us70_th30_stockcap5_penalty002_assets50_segment_cap25"
...
def _segment_groups(selected: list[str], us_assets: set[str], th_assets: set[str]) -> dict[str, list[int]]:
    us_segments, th_segments = _segment_maps()
    ...
    for idx, asset in enumerate(selected):
        ...
        if ticker in us_assets:
            ...
            segment = _normalized_segment(th_segments.get(ticker), "th")
        else:
            segment = None
        if segment:
...
def _write_outputs_with_segment_meta(security: pd.DataFrame, sleeve: pd.DataFrame, meta: pd.DataFrame) -> None:
    if not meta.empty:
        ...
        meta["Segment Cap Mode"] = "combined US+TH normalized segment cap"
        meta["Latest Max Combined Segment Raw Weight"] = (
            max(_latest_raw_segment_weights.values()) if _latest_raw_segment_weights else 0.0
        )
        meta["Latest Combined Segment Weights"] = "; ".join(
            f"{segment}: {weight:.2%}" for segment, weight in sorted(_latest_raw_segment_weights.items())
        )
        meta["Segment Source"] = "data/us_segment.csv and data/set100_segment.xls"
